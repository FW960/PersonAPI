    ...
    public async Task Invoke(HttpContext context)
    {
        if (context.Request.Path.Equals("/authorized.html"))
        {
            try
            {
                ...
                if (!TokenValidator.Validate(token, true))
                {
                    if (TokenValidator.Validate(refreshToken, false))
                    {
                        ...
                        requestMessage.RequestUri = new Uri("https://localhost:7001/authorize/employee/get-new-token");
